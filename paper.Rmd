...
apa_table(t(publicationtype), digits = 0, caption = "Type of document that matched the registration for all projects that were publicly shared")
...
colnames(estimated_nonpublication) <- c("Group", "Registrations", "% Included", "Included", "% Shared", "Publicly Shared", "Not Publicly Shared")

apa_table(longtable = TRUE, landscape = TRUE, estimated_nonpublication, digits = 0, caption = "Descriptives for the sampled registrations, and estimated number of publicly shared and not publicly shared projects registered on the OSF before November 2017.")

# Total published estimate
# estimated_nonpublication$Published[6]
# Total non-published estimate
# estimated_nonpublication$`Not Published`[6]

```

After incorporating responses from researchers, and extrapolating these responses to the registrations for which we remained uncertain if they were published because we did not receive a response (see the supplement for details on the calculation used to resolve the classification for registrations we remained uncertain about), we predict that out of a total of `r sum(included_after$sum_after)` included registrations, `r round(result_total$x, 1)` registered studies are publicly shared (`r round(100*result_total[4], 1)`%, 95% CI [`r round(100*result_total[5], 1)`; `r round(100*result_total[6], 1)`]). Our sample contained an almost equal number of manually opened and automatically opened registrations to examine our research question about the consequences of automatically opening registrations after an embargo. As can be seen in Table \@ref(tab:table-estimated-nonpublication) manually opened registrations were much more likely to be publicly shared than automatically opened registrations. Manually opened registrations were also somewhat less frequent in the total population of registrations compared to automatically opened registrations. Multiplying our publication estimates by the estimated number of registrations on the OSF before November 2017 that can be classified as an actual research study we estimate by extrapolation that `r estimated_nonpublication[6,6]` registrations are publicly shared, and `r estimated_nonpublication[6,7]` remain unpublished. Therefore, in the total population of all registrations on the OSF from before November 2017 we estimated that `r round(100*result_population[4], 1)`% of registrations are publicly shared as an article, preprint, thesis, or poster after at least four years. This extrapolation is based on a simple model which only incorporates difference between manually and automatically opened preregistrations in how often projects are publicly shared, and is based on an estimate of the amount of actual research studies. The extrapolation therefore comes with considerable remaining uncertainty about the true percentages in the population.
...
Our estimate has remaining uncertainty as some registrations for which authors did not respond to our email might be classified incorrectly. Furthermore, the estimate is specific to the context of our study, which were all registrations on the OSF created before November 18, 2017. Users of the OSF in these years are early adopters interested in open science practices, and the likelihood they share a study might differ from researchers who do not use the OSF. Our estimate might not generalize to non-registered studies, nor to educational research projects. The estimate might also change over time (e.g., more studies might have remained unpublished during the COVID pandemic). In short, the exact percentage of studies that remain unpublished will be different in other contexts, such as when the OSF is used more by other disciplines beyond psychology [@vandenakker_selective_2023], when registration is not voluntary, and when preregistration spreads beyond early adopters. At the same time, complementing previous studies [@ofosu_preanalysis_2023; @franco_publication_2014] this study provides converging support for the observation that a substantial amount of research projects are not publicly shared.
